...
[![Smith & Vader Short Demo (30 seconds) Showing Glorious Victory](https://img.youtube.com/vi/aNwIuJUAOQ8/default.jpg)](https://youtu.be/aNwIuJUAOQ8)
...
The ES version of Smith&Vader is noted for it's highly advanced TCP - Tiny Context Protocol - which harnesses the awesome power of the Emperor 8LM, a cutting-edge generative model with exactly 4 braincells (an upgrade from the 2 braincells in the non-ES version). You may now enter into its presence.

Smith&Vader is a raspberry pi OS written in machine language. After the awe-inspiring boot screen you create a user account. Inside you are greeted with a game loosely based on Pac Man. The playfield is populated with random ascii characters. If your in-game character, Peon (Pac Man in a black trench coat) eats an asterick, they will possess the power to devour Agent Smith and Darth Vader! Throughout the game the player is pursued by agents and storm troopers. Eat an "a" to chomp agents and an "s" to devour storm troopers. Any contact with a bad guy will result in death. This will cause the os to reboot and you will be presented with the create user screen. You must create new user, assume the role of Peon, and play again

If you eat Smith and Vader, you win! At this point the TCP requests a "speech" from the 8LM, which generates a death speech from Smith and Vader, stating that you have solved the halting problem. All your previously imprisoned usernames are freed and the os halts, or in other words crashes, and the user is forced to enter ctrl-alt-del, and the game cycle starts over again.

Without further adieu... Everything below here and all code was generated by Claude Code.
